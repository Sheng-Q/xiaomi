...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Serve a live tactile heatmap preview page without saving images.")
...
def make_handler(runtime: TactileRuntime, visualizer: TactileVisualizer, refresh_ms: int, logger: logging.Logger):
...
def main() -> None:
    ...
    try:
        ...
        if runtime.wait_for_frame(timeout=3.0) is None:
            logger.warning("No tactile frames yet, preview page will show a waiting image.")
        elif args.calibrate:
            runtime.calibrate()
        ...
        logger.info("Tactile preview available at http://%s:%s", args.host, args.http_port)
